...
- Digital downloads: MDZ resolver → IIIF presentation manifest → IIIF image API (`api.digitale-sammlungen.de`). Only router-classified `open` links (MDZ, open repositories, direct PDFs) download; portal/aggregator links never do.
- Electronic entitlement (Alma-E/Viewit): `POST pub/edelivery/alma<mms>` is authoritative for `hasAccess`/licence per user. PNX links are classified (`open | resolver | package-portal | aggregator-title | licensed-service | unknown`); the best legal action is `download | open-browser | search-in-portal | request-physical`. No publisher SSO/DRM automation — licensed access is handed to the browser via the resolver URL.
